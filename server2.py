...
def read(data, conn): #i think this is sending it right, just need client side
    ...
    for i in range(0, min_val): #for loop to send data back to client , dont want it to exceed 255
        message_response_2 = bytearray()
        ...
        #print(message_response_2)
        sent = conn.send(message_response_2) #idk whys its sending them togther?????

    '''else:
        message_response = bytearray([0x73, 0xAE, 3, 0, 0, 0, 0])
        sent = conn.send(message_response)'''
